...
{
    return -(u[indPeriodicx(i,j,k)] * (u[indPeriodicx(i+1,j,k)] - u[indPeriodicx(i-1,j,k)]) / (2.0 * DX) +
             (v[indPeriodicy(i,j,k)] + v[indPeriodicy(i+1,j,k)] + v[indPeriodicy(i,j-1,k)] + v[indPeriodicy(i+1,j-1,k)]) / 4.0 * (u[indPeriodicx(i,j+1,k)] - u[indPeriodicx(i,j-1,k)]) / (2.0 * DY) +
             (w[indPeriodicz(i,j,k)] + w[indPeriodicz(i+1,j,k)] + w[indPeriodicz(i,j,k-1)] + w[indPeriodicz(i+1,j,k-1)]) / 4.0 * (u[indPeriodicx(i,j,k+1)/*lu + 1*/] - u[indPeriodicx(i,j,k-1)/*lu - 1*/]) / (2.0 * DZ)) +
           1 / RE * ((u[indPeriodicx(i+1,j,k)] - 2 * u[indPeriodicx(i,j,k)] + u[indPeriodicx(i-1,j,k)]) / (DX * DX) + 
                    (u[indPeriodicx(i,j+1,k)] - 2 * u[indPeriodicx(i,j,k)] + u[indPeriodicx(i,j-1,k)]) / (DY * DY) + 
                    (u[indPeriodicx(i,j,k+1)] - 2 * u[indPeriodicx(i,j,k)] + u[indPeriodicx(i,j,k-1)]) / (DZ * DZ)) +
...
{
    return -((u[indPeriodicx(i,j,k)/*lu*/] + u[indPeriodicx(i,j+1,k)] + u[indPeriodicx(i-1,j,k)] + u[indPeriodicx(i-1,j+1,k)]) / 4.0 * ((v[indPeriodicy(i+1,j,k)] - v[indPeriodicy(i-1,j,k)]) / (2.0 * DX)) +
             v[indPeriodicy(i,j,k)] * (v[indPeriodicy(i,j+1,k)] - v[indPeriodicy(i,j-1,k)]) / (2.0 * DY) +
             (w[indPeriodicz(i,j,k)] + w[indPeriodicz(i,j,k-1)] + w[indPeriodicz(i,j+1,k)] + w[indPeriodicz(i,j+1,k-1)]) / 4.0 * (v[indPeriodicy(i,j,k+1)] - v[indPeriodicy(i,j,k-1)]) / (2.0 * DZ)) +
           (1.0 / RE) * ((v[indPeriodicy(i+1,j,k)] - 2.0 * v[indPeriodicy(i,j,k)] + v[indPeriodicy(i-1,j,k)]) / (DX * DX) +
                         (v[indPeriodicy(i,j+1,k)] - 2.0 * v[indPeriodicy(i,j,k)] + v[indPeriodicy(i,j-1,k)]) / (DY * DY) +
                         (v[indPeriodicy(i,j,k+1)] - 2.0 * v[indPeriodicy(i,j,k)] + v[indPeriodicy(i,j,k-1)]) / (DZ * DZ)) +
...
{
    return -((u[indPeriodicx(i,j,k)] + u[indPeriodicx(i-1,j,k)] + u[indPeriodicx(i,j,k+1)] + u[indPeriodicx(i-1,j,k+1)]) / 4.0 * (w[indPeriodicz(i+1,j,k)] - w[indPeriodicz(i-1,j,k)]) / (2.0 * DX) +
             (v[indPeriodicy(i,j,k+1)] + v[indPeriodicy(i,j-1,k+1)] + v[indPeriodicy(i,j,k)] + v[indPeriodicy(i,j-1,k)]) / 4.0 * (w[indPeriodicz(i,j+1,k)] - w[indPeriodicz(i,j-1,k)]) / (2.0 * DY) +
             w[indPeriodicz(i,j,k)] * (w[indPeriodicz(i,j,k+1)] - w[indPeriodicz(i,j,k-1)]) / (2.0 * DZ)) +
           (1.0 / RE) * ((w[indPeriodicz(i+1,j,k)] - 2.0 * w[indPeriodicz(i,j,k)] + w[indPeriodicz(i-1,j,k)]) / (DX * DX) +
                         (w[indPeriodicz(i,j+1,k)] - 2.0 * w[indPeriodicz(i,j,k)] + w[indPeriodicz(i,j-1,k)]) / (DY * DY) +
                         (w[indPeriodicz(i,j,k+1)] - 2.0 * w[indPeriodicz(i,j,k)] + w[indPeriodicz(i,j,k-1)]) / (DZ * DZ)) +
...
inline int IcoNS::indPeriodicx(int i, int j, int k){
...
inline int IcoNS::indPeriodicy(int i, int j, int k){
    if(k==-1){
        k=dim_z-2;
    }
    if(k==(dim_z)){
...
inline int IcoNS::indPeriodicz(int i, int j, int k){
    return i * newDimY_z * dim_z_z + j * dim_z_z + (k%dim_z_z + dim_z_z) % dim_z_z;
}
