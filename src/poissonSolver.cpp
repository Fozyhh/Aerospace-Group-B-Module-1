...
void PoissonSolver::solveNeumannPoisson(double* F)
{
    ...
    //                 std::cout << F [i * (xSize[0]) * (xSize[1]) + j * (xSize[0]) + k] << " ";
    //             }
    //             std::cout<< std::endl;
    //         }
    //         std::cout<< std::endl;
...
//     //     for (int j = 0; j < zSize[1]; j++) {
//     //         for (int k = 0; k < zSize[2]; k++) {
//     //             pz[j * (zSize[0]) * (zSize[2]) + i * (zSize[2]) + k] /= (2/(DX*DX) * (std::cos((i + c2d->coord[0] * zSize[0]) * M_PI / (NX)) - 1) +
//     //                                                                     2/(DY*DY) * (std::cos((j ) * M_PI / (NY)) - 1) +
//     //                                                                     2/(DZ*DZ) * (std::cos((k + c2d->coord[1] * zSize[1]) * M_PI / (NZ)) - 1));
//     //         }
//     //     }  
//     // }
