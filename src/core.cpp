...
// std::ofstream error_log("../resources/" + error_file);
    ...
    std::cout << "Writing z to file" << std::endl;
    output_z();
    // output_y();
    output_x();
...
{
    MPI_File fh;
    MPI_Offset headerOffset, dataOffsetX, rowOffsetY, colOffsetX, colOffsetY;
    ...
    // Barrier to make sure the header is written before the data
    MPI_Barrier(cart_comm);
    headerOffset += scalarsu.str().size();


    // Write the data
    std::cout << "headerOffset: " << headerOffset << std::endl;
    int count = 0;
    for(size_t i = 0; i < dim_y_x ; i++){
        // Write the whole line of the processor
        int globalStartRow = coords[1] * dim_y_x;
        int globalRowIndex = globalStartRow + i;
        dataOffsetX = headerOffset + globalRowIndex * NX + 1 * sizeof(double);
        std::cout << "Row: "<< i <<std::endl;
        std::cout << "Rank: " << rank << " globalStartRow: " << globalStartRow <<  " globalRowIndex: " << globalRowIndex << " dataOffsetX: " << dataOffsetX << std::endl;
        //Binary write
        //MPI_File_write_at(fh, dataOffsetX, &grid_loc_x[i + zOffset], dim_x_x, MPI_DOUBLE, MPI_STATUS_IGNORE);
        //ASCII write
        std::ostringstream value;
        for(size_t j = 0; j < dim_x_x; j++){
            value << std::fixed << std::setprecision(6) << grid_loc_x[i + zOffset] << " ";
            std::string str = value.str();
            str += "\n";
            //MPI_File_write_at(fh, dataOffsetX + j * 8, str.c_str(), str.size(), MPI_CHAR, MPI_STATUS_IGNORE);
        }
        std::cout << "Value: " << value.str() << std::endl;
    }

    // Barrier to make sure the all the data is written
    MPI_Barrier(cart_comm);
