...
Real IcoNS::L2_error2(const Real t)
{
    Real sum = 0.0;
    
    std::vector<Real> wx(grid.nx, 1.0); 
    wx[0] = 0.5;
    wx[grid.nx - 1] = 0.5;

    std::vector<Real> wy(grid.ny, 1.0); 
    wy[0] = 0.5;
    wy[grid.ny - 1] = 0.5;

    std::vector<Real> wz(grid.nz, 1.0); 
    wz[0] = 0.5;
    wz[grid.nz - 1] = 0.5;
    ...
    for (size_t i = 0; i < grid.nx - 1 ; i++)
    {
        for (size_t j = 0; j < grid.ny - 1 ; j++)
        {
            ...
            {
                size_t lu = i * (grid.ny+1) * (grid.nz+1) + j * (grid.nz+1) + k;
                size_t lv = i * grid.ny * (grid.nz+1) + j * (grid.nz+1) + k;
                size_t lw = i * (grid.ny+1) * grid.nz + j * grid.nz + k;

                Real exactUx = exact_solution.value_x(i + 0.5, j, k, t);
                Real exactUy = exact_solution.value_y(i, j + 0.5, k, t);
                Real exactUz = exact_solution.value_z(i, j, k + 0.5, t);

                sum +=    ((grid.u[lu] - exactUx) * (grid.u[lu] - exactUx) * dx * dy * dz * wx[i] * wy[j] * wz[k])
                        + ((grid.v[lv] - exactUy) * (grid.v[lv] - exactUy) * dx * dy * dz * wx[i] * wy[j] * wz[k])
                        + ((grid.w[lw] - exactUz) * (grid.w[lw] - exactUz) * dx * dy * dz * wx[i] * wy[j] * wz[k]);
            }
        }
    }
