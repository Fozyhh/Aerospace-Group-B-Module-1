   ...
   for (int i = 1 + lbx; i < newDimX_x - 1 - rbx; i++)
   {
       for (int j = 1 + lby; j < newDimY_x - 1 - rby; j++)
       {
           for (int k = 1; k < dim_z - 1; k++)
           {
               ...
               grid.u[getx(i, j, k)] = -50.0 / 120.0 * DT * functionF_u(Y2_x, Y2_y, Y2_z, i, j, k, time + 64.0 / 120.0 * DT);
           }
       }
   }
   ...
   for (int i = 1 + lbx; i < newDimX_y - 1 - rbx; i++)
   {
       for (int j = 1 + lby; j < newDimY_y - 1 - rby; j++)
       {
           for (int k = 1; k < dim_z-1; k++)
           {
               ...
               grid.v[gety(i, j, k)] = -50.0 / 120.0 * DT * functionF_v(Y2_x, Y2_y, Y2_z, i, j, k, time + 64.0 / 120.0 * DT);
           }
       }
   }
   ...
   for (int i = 1 + lbx; i < newDimX_z - 1 - rbx; i++)
   {
       for (int j = 1 + lby; j < newDimY_z - 1 - rby; j++)
       {
...

           {
               ...
               grid.w[getz(i, j, k)] = -50.0 / 120.0 * DT * functionF_w(Y2_x, Y2_y, Y2_z, i, j, k, time + 64.0 / 120.0 * DT);
           }
       }
   }
