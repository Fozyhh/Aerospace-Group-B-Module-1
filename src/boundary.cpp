...
void Boundary::update_boundary(std::vector<double> Yx,std::vector<double> Yy,std::vector<double> Yz, double t){
    ...
    for (size_t j = 1; j < ny; j++)
    {
        ...
        for(size_t k = 1; k < nz; k++)
        {
            ...
            Yx[j*(nz+1) + k] = approximate_boundary_u(0,j,k,t,face);
        }
    ...
    for (size_t j = 1; j < ny; j++)
    {
        ...
        for(size_t k = 1; k < nz; k++)
        {
            ...
            Yx[(nx-1)*(ny+1)*(nz+1) + j*(nz+1) + k] = approximate_boundary_u(nx-1,j,k,t,face);
        }
    ...
    for (size_t i = 1; i < nx; i++)
    {
        ...
        for(size_t k = 1; k < nz; k++)
        {
            ...
            Yy[i * ny * (nz+1) + k] = approximate_boundary_v(i,0,k,t,face);
        }
    }
    ...
    for (size_t i = 1; i < nx; i++)
    {
        ...
        for(size_t k = 1; k < nz; k++)
        {
            ...
            Yy[(ny-1) * (nz+1) + ny * (nz+1) * i + k] = approximate_boundary_v(i,ny-1,k,t,face);
        }
    }
    ...
    for (size_t i = 1; i < nx; i++)
    {
        ...
        {   
            ...
            Yz[(ny+1) * nz * i + j*nz] = approximate_boundary_w(i,j,0,t,face);
        }
    ...
    for (size_t i = 1; i < nx; i++)
    {
        ...
        for (size_t j = 1; j < ny; j++)
        {
            ...
            Yz[(ny+1) * nz * i + j*nz + (nz - 1)] = approximate_boundary_w(i,j,nz-1,t,face);
        }
