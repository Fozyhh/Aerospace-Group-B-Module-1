...
*/
//TODO:
void Boundary::update_boundary_x(std::array<Real,newDimX_x*newDimY_x*dim_z> &Yx, std::array<Real, newDimX_y*newDimY_y*dim_z> &Yy, std::array<Real, newDimX_z*newDimY_z*dim_z_z> &Yz, Real t)
{
...
*/
//TODO:
void Boundary::update_boundary_y(std::array<Real,newDimX_x*newDimY_x*dim_z> &Yx, std::array<Real, newDimX_y*newDimY_y*dim_z> &Yy, std::array<Real, newDimX_z*newDimY_z*dim_z_z> &Yz, Real t)
{
    //UPDATE EVERY INDEX SUCH THAT YOU SKIP FIRST FACE(GHOST)
    //AND THE FIRST AND LAST ROW OF EACH FACE
    size_t face, i, j, k;
    ...
    if(lbx){
        if(lby){ //if it's a processor with coords[1]==0 then it has the front face as boundary
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[((newDimY_y + 1) * dim_z) + k] = boundary_value_u[face]->value(0,0,k,t); //FOR X AND Y WE NEED GLOBAL INDICES OR IT DOESNT WORK
            }
        }

        
        for (size_t j=1 + lby; j < newDimY_y -1 - rby; j++) //Parti da 1(salti ghost) + 1(se devi considerare il boundary) e arrivi a newDimY -1(ghost)-1(se boundary destro)
        {
            face = 4;
            // lby coords = 1 rby coords = 0
            // we move in the opposite direction of the y process count unfortunately
            Yx[((newDimY_y) * dim_z) + j*(NZ+1)] = boundary_value_u[face]->value(0,j + (PY-1-coords[1]) * dim_y_x,0,t);
            face = 0;
            for(size_t k = 1; k < NZ; k++)
            {
                Yx[((newDimY_y) * dim_z) +  j*(NZ+1) + k] = approximate_boundary_u(0,j  + (PY-1-coords[1]) * dim_y_x,k,t,face,1);
            }
            face = 5;
            Yx[((newDimY_y) * dim_z) + j*(NZ+1) + NZ] = boundary_value_u[face]->value(0,j + (PY-1-coords[1]) * dim_y_x,NZ,t);
        }
        ...
        if(rby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[((newDimY_y) * dim_z) + (newDimY_y - 2) * (NZ+1) + k] = boundary_value_u[face]->value(0,NY,k,t);
            }
        }
    }
    //CONTINUE LIKE THIS
    ...
    for (size_t i=1 + lbx; i < newDimX_y - 1 - rbx; i++)
    {
        if(lby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[i * newDimY_y * (NZ+1) +(NZ+1) + k] = boundary_value_u[face]->value(i + coords[0] * dim_x_x,0,k,t);
            }
        }
        ...
        {
            face = 4;
            Yx[newDimY_y * (NZ+1) * i + j*(NZ+1)] = boundary_value_u[face]->value(i + coords[0] * dim_x_x,j + (PY-1-coords[1]) * dim_y_x,0,t);

            face = 5;
            Yx[newDimY_y * (NZ+1) * i + j*(NZ+1) + NZ] = boundary_value_u[face]->value(i + coords[0] * dim_x_x, j + (PY-1-coords[1]) * dim_y_x,NZ,t);
        }
        ...
        if(rby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[newDimY_y * (NZ+1) * i +(newDimY_y-1) * (NZ+1) + k] = boundary_value_u[face]->value(i + coords[0] * dim_x_x,NY,k,t);
            }
        }
    }
    ...
    if(rbx){
        if(lby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[(newDimX_y-2) * newDimY_y * (NZ+1) + (NZ+1) + k] = boundary_value_u[face]->value(NX-1,0,k,t);
            }
        }
        ...
        {
            face = 4;
            Yx[(newDimX_y-2) * newDimY_y * (NZ+1) + j*(NZ+1)] = boundary_value_u[face]->value(NX-1,j + (PY-1-coords[1]) * dim_y_x,0 + (PY-1-coords[1]) * dim_y_x,t);

            face = 1;
            for(size_t k = 1; k < NZ; k++)
            {
                Yx[(newDimX_y-2)*(newDimY_y)*(NZ+1) + j*(NZ+1) + k] = approximate_boundary_u(NX,j + (PY-1-coords[1]) * dim_y_x,k,t,face,-1);
            }

            face = 5;
            Yx[(newDimX_y-2) * (newDimY_y) * (NZ+1) + j*(NZ+1) + NZ] = boundary_value_u[face]->value(NX-1,j + (PY-1-coords[1]) * dim_y_x,NZ,t);
        }
        if(rby){
            face = 3;
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[(newDimX_y-2) * newDimY_y * (NZ+1) + (newDimY_y-2)*(NZ+1) + k] = boundary_value_u[face]->value(NX-1,NY,k,t);
            }
        }
    }
...
*/
//TODO:
void Boundary::update_boundary_z(std::array<Real,newDimX_x*newDimY_x*dim_z> &Yx, std::array<Real, newDimX_y*newDimY_y*dim_z> &Yy, std::array<Real, newDimX_z*newDimY_z*dim_z_z> &Yz, Real t)
{
    //UPDATE EVERY INDEX SUCH THAT YOU SKIP FIRST FACE(GHOST)
    //AND THE FIRST AND LAST ROW OF EACH FACE
    size_t face, i, j, k;
    ...
    if(lbx){
        if(lby){ //if it's a processor with coords[1]==0 then it has the front face as boundary
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[((newDimY_z + 1) * dim_z_z) + k] = boundary_value_u[face]->value(0,0,k,t); //FOR X AND Y WE NEED GLOBAL INDICES OR IT DOESNT WORK
            }
        }

        
        for (size_t j=1 + lby; j < newDimY_z -1 - rby; j++) //Parti da 1(salti ghost) + 1(se devi considerare il boundary) e arrivi a newDimY -1(ghost)-1(se boundary destro)
        {
            face = 4;
            // lby coords = 1 rby coords = 0
            // we move in the opposite direction of the y process count unfortunately
            Yx[((newDimY_z) * dim_z_z) + j*(NZ+1)] = boundary_value_u[face]->value(0,j + (PY-1-coords[1]) * dim_y_x,0,t);
            face = 0;
            for(size_t k = 1; k < NZ; k++)
            {
                Yx[((newDimY_z) * dim_z_z) +  j*(NZ+1) + k] = approximate_boundary_u(0,j  + (PY-1-coords[1]) * dim_y_x,k,t,face,1);
            }
            face = 5;
            Yx[((newDimY_z) * dim_z_z) + j*(NZ+1) + NZ] = boundary_value_u[face]->value(0,j + (PY-1-coords[1]) * dim_y_x,NZ,t);
        }
        ...
        if(rby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[((newDimY_z) * dim_z_z) + (newDimY_z - 2) * (NZ+1) + k] = boundary_value_u[face]->value(0,NY,k,t);
            }
        }
    }
    //CONTINUE LIKE THIS
    ...
    for (size_t i=1 + lbx; i < newDimX_z - 1 - rbx; i++)
    {
        if(lby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[i * newDimY_z * (NZ+1) +(NZ+1) + k] = boundary_value_u[face]->value(i + coords[0] * dim_x_x,0,k,t);
            }
        }
        ...
        {
            face = 4;
            Yx[newDimY_z * (NZ+1) * i + j*(NZ+1)] = boundary_value_u[face]->value(i + coords[0] * dim_x_x,j + (PY-1-coords[1]) * dim_y_x,0,t);

            face = 5;
            Yx[newDimY_z * (NZ+1) * i + j*(NZ+1) + NZ] = boundary_value_u[face]->value(i + coords[0] * dim_x_x, j + (PY-1-coords[1]) * dim_y_x,NZ,t);
        }
        ...
        if(rby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[newDimY_z * (NZ+1) * i +(newDimY_z-1) * (NZ+1) + k] = boundary_value_u[face]->value(i + coords[0] * dim_x_x,NY,k,t);
            }
        }
    }
    ...
    if(rbx){
        if(lby){
            ...
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[(newDimX_z-2) * newDimY_z * (NZ+1) + (NZ+1) + k] = boundary_value_u[face]->value(NX-1,0,k,t);
            }
        }
        ...
        {
            face = 4;
            Yx[(newDimX_z-2) * newDimY_z * (NZ+1) + j*(NZ+1)] = boundary_value_u[face]->value(NX-1,j + (PY-1-coords[1]) * dim_y_x,0 + (PY-1-coords[1]) * dim_y_x,t);

            face = 1;
            for(size_t k = 1; k < NZ; k++)
            {
                Yx[(newDimX_z-2)*(newDimY_z)*(NZ+1) + j*(NZ+1) + k] = approximate_boundary_u(NX,j + (PY-1-coords[1]) * dim_y_x,k,t,face,-1);
            }

            face = 5;
            Yx[(newDimX_z-2) * (newDimY_z) * (NZ+1) + j*(NZ+1) + NZ] = boundary_value_u[face]->value(NX-1,j + (PY-1-coords[1]) * dim_y_x,NZ,t);
        }
        if(rby){
            face = 3;
            for (size_t k=0; k < NZ+1; k++)
            {
                Yx[(newDimX_z-2) * newDimY_z * (NZ+1) + (newDimY_z-2)*(NZ+1) + k] = boundary_value_u[face]->value(NX-1,NY,k,t);
            }
        }
    }
