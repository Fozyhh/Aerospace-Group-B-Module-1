...
Real IcoNS::error_comp_X(const Real t)
{
    ...
    for (int i = 1+lbx; i<newDimX_x-1-rbx; i++)
    {
        for (int j = 1+lby; j<newDimY_x-1-rby; j++)
        {
            ...
            {
                error += ((grid.u[getx(i, j, k)] - exact_solution.value_x(i+offset_x+0.5, j+offset_y, k, t)) *
                          (grid.u[getx(i, j, k)] - exact_solution.value_x(i+offset_x+0.5, j+offset_y, k, t)) *
                          DX * DY * DZ);
            }
        }
    }
...
Real IcoNS::error_comp_Y(const Real t)
{
    ...
    for (int i = 1+lbx; i<newDimX_y-1-rbx; i++)
    {
        for (int j = 1+lby; j<newDimY_y-1-rby; j++)
        {
            for (int k = lbz; k<dim_z-rbz; k++)
            {
                error += ((grid.v[gety(i, j, k)] - exact_solution.value_y(i+offset_x, j+offset_y+0.5, k, t)) *
                          (grid.v[gety(i, j, k)] - exact_solution.value_y(i+offset_x, j+offset_y+0.5, k, t)) *
                          DX * DY * DZ);
            }
        }
    }
...
Real IcoNS::error_comp_Z(const Real t)
{
    ...
    for (int i = 1+lbx; i<newDimX_z-1-rbx; i++)
    {
        for (int j = 1+lby; j<newDimY_z-1-rby; j++)
        {
            ...
            {
                error += ((grid.w[getz(i, j, k)] - exact_solution.value_z(i+offset_x, j+offset_y, k+0.5, t)) *
                          (grid.w[getz(i, j, k)] - exact_solution.value_z(i+offset_x, j+offset_y, k+0.5, t)) *
                          DX * DY * DZ);
            }
        }
    }
...
{
    Real error = 0.0;
    int offset_x = coords[0] * xSize[2] + std::max(0, coords[0] - (PX - (NX + 1) % PX))-1;
    int offset_y = coords[1] * xSize[1] + std::max(0, coords[1] - (PY - (NY + 1) % PY))-1;
    for (int i = 1+lbx; i<xSize[2]+1-rbx; i++)
    {
        for (int j = 1+lby; j<xSize[1]+1-rby; j++)
        {
            for (int k = lbz; k<xSize[0]-rbz; k++)
            {
                error += ((grid.p[getHaloP(i, j, k)] - exact_solution.value_p(i+offset_x, j+offset_y, k, t)) *
                          (grid.p[getHaloP(i, j, k)] - exact_solution.value_p(i+offset_x, j+offset_y, k, t)) *
                          DX * DY * DZ);
            }
        }
    }
