...
int main(int argc, char **argv)
{
    ...
    */
    ...
    MPI_Type_vector(dim_y, dim_z, res_y, MPI_INT, &MPI_face);
    ...
    if (neighbors[0] != -2)
    {
        MPI_Send(&grid_loc[newDimY * dim_z + dim_z + 1], 1, MPI_face, neighbors[0], rank, cart_comm);
    }

    if (neighbors[2] != -2)
    {
        MPI_Recv(&grid_loc[(newDimX - 1) * newDimY * dim_z + dim_z + 1], 1, MPI_face, neighbors[2], neighbors[2], cart_comm, &status);
    }
